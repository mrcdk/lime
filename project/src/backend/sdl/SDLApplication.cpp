...
namespace lime {
	...
	SDLApplication::SDLApplication () {
		...
		printf("SDL_HINT_IOS_HIDE_HOME_INDICATOR set to: %s\n", SDL_GetHint(SDL_HINT_IOS_HIDE_HOME_INDICATOR));
		#endif
		...
		if (SDL_Init (SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_GAMECONTROLLER | SDL_INIT_TIMER | SDL_INIT_JOYSTICK | SDL_WINDOW_ALLOW_HIGHDPI) != 0) {
	...
	void SDLApplication::SetEmulatedMouseFromTouch(bool enabled) {
		emulateMouseFromTouch = enabled;
	}

	bool SDLApplication::GetEmulatedMouseFromTouch() {
